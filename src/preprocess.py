...
def main():
    ...
    df["Grade_Group"] = df["Next_Grade"].apply(
    ...
    # -------------------------------
    final_cols = [
        "Overall_lag1", "Overall_lag2", "Overall_trend",
        "Outcome_lag1", "Outcome_lag2", "Outcome_trend",
        "ECT_lag1", "ECT_lag2",
        "IFSE_lag1", "IFSE_lag2",
        "SSCP_lag1", "SSCP_lag2",
        "DL_lag1", "DL_lag2",
        "GP_lag1", "GP_lag2",
        "Year", "District_Encoded", "Next_Grade","Grade_Group"
    ]
    ...
    df.to_csv(PROCESSED_PATH / "clean_pgi_data.csv", index=False)
