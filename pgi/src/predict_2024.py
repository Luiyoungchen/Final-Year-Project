...
def predict(df_features, model):
    ...
    df_features["Predicted_Grade_Num"] = model.predict(X).astype(int)
    df_features["Predicted_Grade_Label"] = df_features["Predicted_Grade_Num"].map(GRADE_LABELS)
...
def main():
    ...
    print(predictions[["District", "Predicted_Grade_Num", "Predicted_Grade_Label"]].head(10))
